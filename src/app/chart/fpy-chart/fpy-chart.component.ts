...
@Component({
  selector: 'app-fpy-chart',
  templateUrl: './fpy-chart.component.html',
  styleUrls: ['./fpy-chart.component.scss']
})
export class FpyChartComponent implements OnInit, OnChanges, ChartComponentBase {

  public title = 'FPY Chart';
  public data: any;
  ...
  fpyChartDiscreteChartDataList: DiscreteData[];
  ...
  ngOnInit(): void {
    this.fpyChartData$.subscribe(fpyChartData => {
      ...
      if (isNullObject(this.pageDiscreteChartData)) {
        this.fpyChartDiscreteChartDataList = this.pageDiscreteChartData.discreteDataList;
        if (isEmptyArray(this.fpyChartDiscreteChartDataList)) {
          const labels = [];
          ...
          const target = [];
          const cl = [];
          this.fpyChartDiscreteChartDataList.forEach( (discreteData) => {
            labels.push(discreteData.seq);
            firstPassYield.push('100');
            ucl.push(discreteData.ucl);
            lcl.push(discreteData.lcl);
            target.push(this.pageDiscreteChartData.target);
            cl.push(this.pageDiscreteChartData.cl);
          });
          this.data = Object.assign({
            y: 'Defects',
            series: [
              {name: 'First Pass Yield %', values: firstPassYield, visibility: true},
              {name: 'UCL', values: ucl, visibility: true},
              {name: 'LCL', values: lcl, visibility: true},
              {name: 'Target', values: target, visibility: true},
  ...
  public drawAxis(): void {
    ...
    this.z.domain(['First Pass Yield %', 'UCL', 'LCL', 'Target', 'CL']);
  ...
  public drawLegend(): void {
    ...
    const itemPadding = 5;
    const textOffset = 2;
    const domains = ['First Pass Yield %', 'UCL', 'LCL', 'Target', 'CL'];
    legend.append('rect')
      .attr('width', 190)
      .attr('height', 145)
      .style('fill', 'none')
