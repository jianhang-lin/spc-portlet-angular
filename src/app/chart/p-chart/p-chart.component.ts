...
export class PChartComponent implements OnInit, OnChanges, ChartComponentBase {

  public title = 'P Chart';
  public data: any;
  ...
  pChartDiscreteChartDataList: DiscreteData[];
  ...
  constructor(public router: Router,
    ...
    this.width = 1200;
    this.height = 400;
    this.store$.dispatch(new pChartAction.LoadPChartDataAction(null));
  ...
  ngOnInit(): void {
    this.pChartData$.subscribe(pChartData => {
      ...
      if (isNullObject(this.pageDiscreteChartData)) {
        this.pChartDiscreteChartDataList = this.pageDiscreteChartData.discreteDataList;
        if (isEmptyArray(this.pChartDiscreteChartDataList)) {
          const labels = [];
          const ratio = [];
          const ucl = [];
          const lcl = [];
          const cl = [];
          this.pChartDiscreteChartDataList.forEach( (discreteData) => {
            labels.push(discreteData.seq);
            ratio.push(discreteData.defective);
